...
async def send_verification_email(to_email: str, code: str) -> None:
    ...
    if settings.resend_api_key:
        await _send_via_resend(to_email, code, settings.resend_api_key, settings.email_from)
...
async def _send_via_resend(to_email: str, code: str, api_key: str, from_address: str) -> None:
    async with httpx.AsyncClient() as client:
        response = await client.post(
            "https://api.resend.com/emails",
            headers={"Authorization": f"Bearer {api_key}"},
            json={
                "from": from_address or "DI Indmeldelses Portal <onboarding@resend.dev>",
                "to": [to_email],
